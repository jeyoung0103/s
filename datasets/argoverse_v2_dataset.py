...
class ArgoverseV2Dataset(Dataset):
    ...
    def __init__(self,
        ...
        if processed_dir is None:
            ...
            if os.path.isdir(self._processed_dir):
                self._processed_file_names = sorted([name for name in os.listdir(self._processed_dir) if
    ...
    def get_agent_features(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        # Filter out static agents (keep only moving agents + riderless_bicycle)
        # Static agents: static (5), background (6), construction (7), unknown (9) are filtered out
        # Riderless_bicycle (index 8) will be mapped to cyclist later in Graphormer
        filtered_agent_ids = []
        for track_id in agent_ids:
            track_df = df[df['track_id'] == track_id]
            if len(track_df) > 0:
                agent_type_str = track_df['object_type'].values[0]
                try:
                    agent_type_idx = self._agent_types.index(agent_type_str)
                    # Filter out static agents: static (5), background (6), construction (7), unknown (9)
                    if agent_type_idx not in [5, 6, 7, 9]:  # static, background, construction, unknown
                        filtered_agent_ids.append(track_id)
                except ValueError:
                    # AV or other special agents - keep them
                    filtered_agent_ids.append(track_id)
        ...
        if 'AV' in agent_ids:
            ...
            av_idx = 0  # Fallback if AV not found
    ...
    def len(self) -> int:
        # Return the actual number of processed files if they exist, otherwise return the expected number
        if len(self._processed_file_names) > 0:
