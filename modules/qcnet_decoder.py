...
class QCNetDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.scene_query_gate = nn.Parameter(torch.zeros(1))
        self.apply(weight_init)

    def forward(self,
                data: HeteroData,
                scene_enc: Mapping[str, torch.Tensor],
                scene_token: Optional[torch.Tensor] = None) -> Dict[str, torch.Tensor]:
        ...
        if scene_token is not None:
            m = m + torch.sigmoid(self.scene_query_gate) * scene_token.repeat_interleave(self.num_modes, dim=0)
